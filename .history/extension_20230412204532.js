      ...
      if (editor.document.lineAt(i).text.includes("//") && convertible) {
        ...
        lines.push(
          `std::ostringstream ${varname}; ${varname} << ` +
            ...
            `
			printf("_rescount_${i}_/rescount__res_%s_/res_", ${varname}.str().c_str());`
        );
      ...
      if (err) {
        ...
        child_process.execFile(exe_file, (err, stdout, stderr) => {
          if (err) {
            ...
            let outres = stdout.match(
              /_rescount_(\d+)_\/rescount__res_(.*?)\/res_/g
            );
            for (let i = 0; i < outres.length; i++) {
              let line = outres[i].match(
                /_rescount_(\d+)_\/rescount__res_(.*?)\/res_/
              );
              setTextAtLine(editor, line[1], line[2]);
            }
