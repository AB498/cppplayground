  ...
  vscode.workspace.onDidChangeTextDocument((event) => {
    ...
    for (let i = 0; i < editor.document.lineCount; i++) {
      ...
      if (editor.document.lineAt(i).text.includes("//") && convertible) {
        // get number after // but before line break
        let numberAfterSlash =
          parseInt(editor.document.lineAt(i).text.match(/\/\/(.*)/)[1]) || 0;
        console.log(numberAfterSlash);
        let varname = "os_" + i;
        lines.push(
          `auto ${varname} = ` +
            editor.document.lineAt(i).text +
            `
    printf("_rescount_${i}_/rescount__res_%s_/res_", ConvToString(${varname}${
              numberAfterSlash != 0 ? "," + numberAfterSlash : ""
            }).c_str());
	`
        );
